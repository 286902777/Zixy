...
extension Notification.Name {
    static let zixyUserProfileDidChange = Notification.Name(
        "zixy_user_profile_did_change"
    )
}

enum ZixyUserAvatarStore {

    private static let localReferencePrefix = "zixy_local_avatar_"
    private static let directoryName = "UserAvatars"

    static func image(for user: ZixyUserRecord) -> UIImage? {
        image(reference: user.avatarAssetName)
    }
    ...
    static func image(reference: String) -> UIImage? {
        ...
        guard let url = try? avatarDirectoryURL().appendingPathComponent(
            reference,
            isDirectory: false
        ) else {
            return nil
    ...
    static func save(_ image: UIImage) throws -> String {
        ...
        let url = try avatarDirectoryURL().appendingPathComponent(
            reference,
            isDirectory: false
        )
